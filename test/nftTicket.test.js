...
describe("NFTTicket", function () {
  ...
  describe("Event Creation", function () {
    it("Creates an event successfully", async function () {
      const tx = await contract
        .connect(organiser)
        .createEvent("QmFakeIpfsHash123", 100, ONE_ETH, ROYALTY_BPS);
    ...
    it("Reverts with empty IPFS hash", async function () {
      await expect(
        contract.connect(organiser).createEvent("", 10, ONE_ETH, ROYALTY_BPS)
  ...
  describe("Edit Event", function () {
    beforeEach(async function () {
      await contract
        .connect(organiser)
        .createEvent("QmOriginalHash", 50, ONE_ETH, ROYALTY_BPS);
    });
    ...
    it("Organiser can edit their event", async function () {
      ...
      const tx = await contract
        .connect(organiser)
        .editEvent(1, "QmNewHash", 100, newPrice);
      const receipt = await tx.wait();
      console.log("  ⛽  editEvent gas:", receipt.gasUsed.toString());

      const evt = await contract.fetchEventData(1);
      expect(evt.maxTickets).to.equal(100);
      expect(evt.priceWei).to.equal(newPrice);
    });

    it("Non-organiser cannot edit event", async function () {
      await expect(
        contract.connect(buyer).editEvent(1, "QmHack", 100, ONE_ETH)
  ...
  describe("Ticket Purchase", function () {
    beforeEach(async function () {
      await contract
        .connect(organiser)
        .createEvent("QmFakeIpfsHash123", 10, ONE_ETH, ROYALTY_BPS);
    });

    it("Buys a single Silver ticket", async function () {
      const tx = await contract
        .connect(buyer)
        .buyTicket(1, 1, TIER_SILVER, { value: ONE_ETH });
      const receipt = await tx.wait();
    ...
    it("Buys multiple VIP tickets", async function () {
      ...
      const tx = await contract
        .connect(buyer)
        .buyTicket(1, qty, TIER_VIP, { value: ONE_ETH * BigInt(qty) });
      const receipt = await tx.wait();
    ...
    it("Buying beyond max tickets reverts", async function () {
      ...
      await contract
        .connect(buyer)
        .buyTicket(1, 10, TIER_SILVER, { value: ONE_ETH * 10n });
      // Try to buy one more
      await expect(
        contract
          .connect(buyer2)
  ...
  describe("Batch Ticket Purchase", function () {
    beforeEach(async function () {
      await contract
        .connect(organiser)
        .createEvent("QmBatchEvent", 50, ONE_ETH, ROYALTY_BPS);
    });
  ...
  describe("Marketplace & Royalties", function () {
    const RESALE_PRICE = ONE_ETH;

    beforeEach(async function () {
      await contract
        .connect(organiser)
        .createEvent("QmConcertHash", 10, ONE_ETH, ROYALTY_BPS);
    ...
    it("Lists a ticket for resale (gas)", async function () {
      ...
      expect(listing.priceWei).to.equal(RESALE_PRICE);
    });

    it("A non-owner cannot list a ticket for resale", async function () {
      await expect(
        contract.connect(buyer2).listForResale(1, ONE_ETH)
      ).to.be.revertedWith("Not the owner");
    });

    it("Buys a resale ticket with correct royalty split (gas)", async function () {
      await contract.connect(buyer).listForResale(1, RESALE_PRICE);
      ...
      const tx = await contract
        .connect(buyer2)
        .buyResaleTicket(1, { value: RESALE_PRICE });
      const receipt = await tx.wait();
      ...
      // 10 % of 1 ETH = 0.1 ETH royalty  (ROYALTY_BPS = 1000)
    ...
    it("A cancelled listing cannot be purchased", async function () {
      ...
      await expect(
        contract.connect(buyer2).buyResaleTicket(1, { value: ONE_ETH })
    ...
    it("Resale buyer can relist at higher price", async function () {
      ...
      const higherPrice = ethers.parseEther("2");
      await contract.connect(buyer2).listForResale(1, higherPrice);

      const listing = await contract.getResaleListing(1);
      expect(listing.active).to.equal(true);
      expect(listing.seller).to.equal(buyer2.address);
      expect(listing.priceWei).to.equal(higherPrice);
    });
  });
});